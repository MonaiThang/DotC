...
public class ManageRecord {
	...
	public String diagRecord() throws Exception {
		...
		Query<Record> q = ds.createQuery(Record.class)
				.field("PatientCitizenID")
				.equal(this.record.getPatientCitizenID()).order("-RegisDate")
				.limit(1);
		UpdateOperations<Record> ops = ds.createUpdateOperations(Record.class)
				.set("Medication", this.record.getMedication())
				.set("Diagnosis", this.record.getDiagnosis())
				.set("DiagDate", EditDate).set("timestamp", EditDate);
		// Save the POJO
	...
	public String searchRecord() throws Exception {
		...
		Query<Record> q = ds.createQuery(Record.class);
		// Create criteria
		q.or(q.criteria("PatientCitizenID").equal(
				this.record.getPatientCitizenID()),
				q.criteria("PatientFirstName").equal(
						this.record.getPatientFirstName()),
				q.criteria("PatientLastName").equal(
						this.record.getPatientLastName()),
				q.criteria("DoctorFirstName").equal(
						this.record.getDoctorFirstName()),
				q.criteria("DoctorLastName").equal(
						this.record.getDoctorLastName()));
		querySet = q.asList();
		// System.out.println("Query Result:");
		// for (Record r : querySet) {
		// System.out.println(r.getPatientCitizenID() + " "
		// + r.getPatientFirstName() + " " + r.getPatientLastName()
		// + " " + r.getDoctorFirstName() + " "
		// + r.getDoctorLastName());
		// }
		return null;
	}
	...
	public String showRecord() throws Exception {
		...
		Datastore ds = morphia.createDatastore(mongo, "dotc");
		// Create query
		Query<Record> q = ds.createQuery(Record.class).field("RecordID")
				.equal(this.record.getRecordID());
		// Create criteria
		record = q.get();
		// System.out.println("Query Result:");
