...
public class ManagePrescription {
	...
	public String listPrescription() throws Exception {
		...
		q.or(q.criteria("PrescriptionID").equal(
				this.prescription.getPrescriptionID()),
				q.criteria("PatientID").equal(this.prescription.getPatientID()),
				q.criteria("PatientFirstName").equal(
						this.prescription.getPatientFirstName()),
				q.criteria("PatientLastName").equal(
						this.prescription.getPatientLastName()),
				q.criteria("DoctorID").equal(this.prescription.getDoctorID()),
				q.criteria("DoctorFirstName").equal(
						this.prescription.getDoctorFirstName()),
				q.criteria("DoctorLastName").equal(
						this.prescription.getDoctorLastName()),
				q.criteria("PrescribeDate").greaterThanOrEq(
						this.prescription.getPrescribeDate()),
				q.criteria("timestamp").greaterThanOrEq(
						this.prescription.getTimestamp()), q.criteria("Status")
						.equal(this.prescription.getStatus()));
		querySet = q.asList();
		// System.out.println("Query Result:");
		// for (Prescription p : querySet) {
		// System.out.println(p.getPrescriptionID() + " " + p.getPatientID()
		// + " " + p.getPatientFirstName() + " "
		// + p.getPatientLastName() + " " + p.getRawStringList() + " "
		// + p.getDoctorFirstName() + " " + p.getDoctorLastName()
		// + " " + p.getStatus());
		// }
	...
	public String changePrescriptionStatus() throws Exception {
		...
		Query<Prescription> q = ds.createQuery(Prescription.class)
				...
				.equal(this.prescription.getPrescriptionID());
		UpdateOperations<Prescription> ops = ds
				.createUpdateOperations(Prescription.class)
				.set("Status", this.prescription.getStatus())
